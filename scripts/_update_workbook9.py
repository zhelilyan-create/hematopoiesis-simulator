"""
Append 27 epigenetic activation test rows (run_ids 207-233) to calibration workbook.
Series 18 — wde x wle x seeds, total_shift=0.04 (noise=0.02, asym=0.02).
"""
import json, re
...
SWEEP_OUT     = "scripts/_sweep9_out.txt"

FIXED = dict(
    mode_inh="centriole", t_max=100.0, init_hsc=10,
    cent_stem=0.015, cent_stress=0.01, age_cap=10,
    stress_acc=0.001, stem_drift=0.0,
    w_div_stemness=1.0, w_div_stress=0.25, w_div_repl=0.005,
    w_diff_stress=0.1, w_diff_stemness=1.0, w_diff_repl=0.005,
    w_apo_stress=1.0, w_apo_stemness=0.0, w_apo_repl=0.005,
    epi_noise=0.02, epi_asym=0.02,
    series="v0.7_epigenetic_activation_test",
    hypothesis="non-zero epigenetic_bias enables measurable rate modulation",
)
...
ws.cell(insert_row, 1).value = (
    "SERIES 18 — v0.7 epigenetic activation test, "
    "wde=[0.0,0.03,0.06] x wle=[0.0,0.05,0.10], noise=asym=0.02, seeds=[42,1,99]"
)
insert_row += 1
...
for r in runs:
    f01 = r.get("bias_f01", 0.0)
    f02 = r.get("bias_f02", 0.0)
    std = r.get("bias_std", 0.0)
    notes_val = (
        f"wde={r['wde']}_wle={r['wle']}_"
        f"std={std:.3f}_f>01={f01:.1f}%_f>02={f02:.1f}%_{r['status']}"
    )
    ...
    set_cell(insert_row, "notes",            notes_val)
