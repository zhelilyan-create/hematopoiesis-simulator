"""
v0.7 lineage-epigenetic test:
  w_lineage_epigenetic in {0.0, 0.03, 0.05, 0.07}, seeds {42, 1, 99} → 12 runs
  w_diff_epigenetic fixed at 0.0 (isolate lineage effect only).
  All other params from baseline YAML.
"""
import subprocess, json, sys, re

BASELINE_CONFIG = "configs/hematopoiesis_baseline.yaml"
WLE_VALUES = [0.0, 0.03, 0.05, 0.07]
SEEDS = [42, 1, 99]
...
results = []
run_id_start = 168  # next after series 15 (154-167)

print(f"{'rid':>4}  {'wle':>5}  {'seed':>4} | {'n':>6}  {'hsc%':>5}  {'mpp%':>5}  {'clp%':>5}  {'cmp%':>5}  {'my%':>5}  {'ery%':>5}  {'mat%':>5} | status")
print("-" * 95)

for i, wle in enumerate(WLE_VALUES):
    for j, seed in enumerate(SEEDS):
        rid = run_id_start + i * len(SEEDS) + j
        stdout = run_one(wle, seed)
        m = parse_output(stdout) if stdout else {}
        status = classify(m)
        results.append(dict(run_id=rid, wle=wle, seed=seed, status=status, **m))
        ...
        my   = m.get("Myeloid_pct", float("nan"))
        ery  = m.get("Erythroid_pct", float("nan"))
        mat  = m.get("mature_pct", float("nan"))
        print(f"{rid:>4}  {wle:>5.2f}  {seed:>4} | {n:>6}  {hpct:>5.1f}  {mpct:>5.1f}  {clp:>5.1f}  {cmp_:>5.1f}  {my:>5.1f}  {ery:>5.1f}  {mat:>5.1f} | {status}")

    print()  # blank line between wle groups
