"""
v0.7 final validation: A (wde=0.0) vs B (wde=0.03), 7 seeds each → 14 runs.
...
def parse_output(stdout):
    ...
    if cell_counts:
        for ct in ["HSC", "MPP", "CLP", "CMP", "B_cell", "T_cell", "Myeloid", "Erythroid"]:
            metrics[ct]          = cell_counts.get(ct, 0)
            metrics[f"{ct}_pct"] = cell_pcts.get(ct, 0.0)
...
results = []
run_id_start = 154  # next after series 14 (145-153)

print(f"{'rid':>4}  {'mode':>4}  {'wde':>5}  {'seed':>4} | {'n':>6}  {'hsc%':>5}  {'mpp%':>5}  {'mat%':>5}  {'ratio':>6} | status")
print("-" * 80)

for mode_label, wde in MODES:
    for seed in SEEDS:
        rid = run_id_start
        run_id_start += 1
        ...
        status = classify(m)
        results.append(dict(run_id=rid, mode=mode_label, wde=wde, seed=seed, status=status, **m))
        ...
        print(f"{rid:>4}  {mode_label:>4}  {wde:>5.2f}  {seed:>4} | {n:>6}  {hpct:>5.1f}  {mpct:>5.1f}  {mat:>5.1f}  {ratio:>6.3f} | {status}")
