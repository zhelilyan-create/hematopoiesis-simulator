"""
Append 9 local refinement sweep rows (run_ids 127-135, csf=0.01) to calibration workbook.
"""
import json, re
...
SWEEP_OUT     = "scripts/_sweep3_out.txt"
...
FIXED = dict(
    ...
    w_apo_stemness=0.0,
    w_apo_repl=0.005,
    series="local_refinement_csf_0.01",
    hypothesis="increase maturation within stable regime via w_div_stress",
)
...
ws.cell(insert_row, 1).value = (
    "SERIES 12 — Local refinement sweep csf=0.01, wds=[0.25,0.27,0.29], seeds=[42,1,99]"
)
insert_row += 1
...
def baseline_flag(r):
    # wds=0.25, seed=42 is a re-run of the exact baseline point
