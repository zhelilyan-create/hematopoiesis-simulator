"""
Append 14 validation rows (run_ids 154-167) to calibration workbook.
Mode A = wde=0.0 (current_best), Mode B = wde=0.03 (candidate).
"""
import json, re
...
SWEEP_OUT     = "scripts/_sweep6_out.txt"
...
FIXED = dict(
    ...
    w_apo_stemness=0.0,
    w_apo_repl=0.005,
    series="v0.7_baseline_vs_epigenetic_validation",
    hypothesis="w_diff_epigenetic=0.03 improves stability without harming maturation",
)
...
ws.cell(insert_row, 1).value = (
    "SERIES 15 — v0.7 A vs B validation, w_diff_epigenetic=[0.0,0.03], seeds=[0,1,2,3,4,42,99]"
)
insert_row += 1
...
for r in runs:
    flag = "current_best" if r["mode"] == "A" else "candidate"
    # embed mode label in notes
    notes_val = f"mode_{r['mode']}_{r['status']}"
    ...
    set_cell(insert_row, "baseline_flag",    flag)
