"""
Append 9 v0.7 epigenetic effect test rows (run_ids 136-144) to calibration workbook.
"""
import json, re
...
SWEEP_OUT     = "scripts/_sweep4_out.txt"

# Baseline fixed params (all from hematopoiesis_baseline.yaml)
FIXED = dict(
    mode="centriole",
    t_max=100.0,
    init_hsc=10,
    ...
    w_apo_stemness=0.0,
    w_apo_repl=0.005,
    series="v0.7_epigenetic_effect_test_1",
    hypothesis="epigenetic_bias affects differentiation via w_diff_epigenetic",
)
...
ws.cell(insert_row, 1).value = (
    "SERIES 13 — v0.7 epigenetic effect test, w_diff_epigenetic=[0.0,0.05,0.1], seeds=[42,1,99]"
)
insert_row += 1
