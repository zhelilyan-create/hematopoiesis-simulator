"""
Append 27 epigenetic bias calibration rows (run_ids 180-206) to calibration workbook.
Series 17 — inheritance_noise x asymmetry_strength x seeds.
"""
import json, re
...
SWEEP_OUT     = "scripts/_sweep8_out.txt"
...
FIXED = dict(
    ...
    series="v0.7_epigenetic_bias_calibration",
    hypothesis="epigenetic parameters control bias amplitude distribution",
)
...
ws.cell(insert_row, 1).value = (
    "SERIES 17 — v0.7 epigenetic bias calibration, "
    "noise=[0.01,0.03,0.05] x asym=[0.01,0.03,0.05], seeds=[42,1,99]"
)
insert_row += 1
...
for r in runs:
    ...
    set_cell(insert_row, "baseline_flag",    "calibration")
