"""
Append 9 local refinement sweep rows (run_ids 118-126) to calibration workbook.
"""
import json, re
import openpyxl

WORKBOOK_PATH = "results/calibtations/hemotopoiesis_calibration.xlsx"
SWEEP_OUT     = "scripts/_sweep2_out.txt"

# Baseline fixed params (from hematopoiesis_baseline.yaml, except csf/wds which vary)
FIXED = dict(
    mode="centriole",
    t_max=100.0,
    init_hsc=10,        # baseline YAML initial HSC
    cent_stem=0.015,    # centriole_stemness_factor from baseline
    age_cap=10,
    stress_acc=0.001,
    stem_drift=0.0,
    w_div_stemness=1.0,
    ...
    w_apo_stemness=0.0,
    w_apo_repl=0.005,
    series="local_refinement_csf_0.015",
    hypothesis="stability check around high differentiation regime",
)

# Read JSON from sweep output
...
print("Existing columns:", list(col_idx.keys()))
print(f"Current last row: {ws.max_row}")

# Section header row
insert_row = ws.max_row + 1
ws.cell(insert_row, 1).value = "SERIES 11 — Local refinement sweep csf=0.015, wds=[0.28,0.30,0.32], seeds=[42,1,99]"
insert_row += 1
...
for r in runs:
    ...
    set_cell(insert_row, "mode",          FIXED["mode"])
    ...
    set_cell(insert_row, "cent_stress",   r["csf"])
    ...
    set_cell(insert_row, "w_div_stress",  r["wds"])
    ...
    set_cell(insert_row, "notes",         r["status"])
    ...
    set_cell(insert_row, "baseline_flag", baseline_flag(r))
...
flags = [ws.cell(r, col_idx["baseline_flag"]).value
